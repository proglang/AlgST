...
runBenchmarkable' :: Benchmarkable
             ...
             -- should take.
             -> IO (Maybe (V.Vector Measured, Double, Int64))
runBenchmarkable' bm minDuration minSamples timeLimit = do
  ...
  -- The first run may only run for 2 minutes. If it is slower we abort this
  -- benchmark.
  m1 <- timeout 120_000_000 $ measure (iterateBenchmarkable bm 1) >>= evaluate
  case m1 of
    Just m -> do
      _ <- note "first iteration completed in time; "
      let valid = measTime m >= milliSecondsToDouble minDuration
      Just <$> loop
        (tail (squish (unfoldr series 1)))
        (if valid then 1 else 0)
        (if valid then [m] else [])
    Nothing -> do
      pure $ Nothing
...
runBenchmarkable :: String -> Benchmarkable -> Gauge (Maybe (V.Vector Measured))
runBenchmarkable desc bm = do
    cfg@Config{..} <- askConfig
    case measureWith of
        Just prog -> gaugeIO $ Just <$> runBenchmarkIsolated cfg prog desc
  ...
  where
    runNormal cfg@Config{..} = do
        ...
        i0 <- if includeFirstIter
                then return $ Just 0
                else timeout 120_000_000 $ iterateBenchmarkable_ bm 1 >> return 1
        let limit = bmTimeLimit cfg
        r <- case i0 of
          Nothing -> return Nothing
          Just _ -> runBenchmarkable' bm minDuration (bmMinSamples cfg) limit
        case r of
          Just (meas, timeTaken, i) -> do
            when ((verbosity == Verbose || not quickMode) && timeTaken > limit * 1.25) .
                void $ prolix "took %s, total %d iterations\n" (secs timeTaken) (fromMaybe 0 i0 + i)
            writeMeas csvRawFile desc meas
            return (Just meas)
          Nothing -> do
            _ <- prolix "aborted after 2 minutes\n\n"
            return Nothing
    ...
    for selector bs $ \_idx desc bm ->
        case analyse of
            BenchmarkNormal step ->
                runBenchmarkable desc bm >>= maybe (pure ()) (step desc >=> \_ -> return ())
